...
- CMake creates a build-local U++ package copy at `build/upp-nest/Croon` so the lowercase repository directory can still expose package `Croon` to `umk` without path-case warnings.
